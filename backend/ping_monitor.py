...
if __name__ == "__main__":
    ...
    print('[Monitor] Iniciado (SLA + hourly stats activos)')
    ...
    while True:
        ...
        for r in routers:
            ...
            if res is None or res is False:
                latency = 0; packet_loss = 100.0; anomaly = True
                print(f'{r.hostname}: SIN RESPUESTA')
            else:
                latency = res * 1000; packet_loss = 0.0; anomaly = latency > threshold
                print(f'{r.hostname}: {latency:.2f}ms')

            if r.id not in hourly_buffer:
                hourly_buffer[r.id] = {'pings': [], 'downtime': 0}
            hourly_buffer[r.id]['pings'].append({'latency': latency, 'packet_loss': packet_loss, 'anomaly': anomaly})
            if packet_loss == 100:
                hourly_buffer[r.id]['downtime'] += interval

            prev = router_state.get(r.id)
            if prev is None and anomaly is True:
                Incident.objects.create(router=r, type='down', started_at=djtime.now(), detail=f'Latencia: {latency:.1f}ms, Perdida: {packet_loss}%')
                print(f'[INCIDENTE] {r.hostname} CAIDA (inicio)')
            elif prev is False and anomaly is True:
                Incident.objects.create(router=r, type='down', started_at=djtime.now(), detail=f'Latencia: {latency:.1f}ms, Perdida: {packet_loss}%')
                print(f'[INCIDENTE] {r.hostname} CAIDA')
            elif prev is True and anomaly is False:
                last_inc = Incident.objects.filter(router=r, ended_at__isnull=True).order_by('-started_at').first()
                if last_inc:
                    ...
                    last_inc.ended_at = recovery_time; last_inc.duration = dur
                    last_inc.save(update_fields=['ended_at', 'duration'])
                    rec = 'recovery' if last_inc.type == 'down' else 'sla_ok'
                    Incident.objects.create(router=r, type=rec, started_at=recovery_time, duration=dur, detail=f'Recuperado tras {dur}')
                print(f'[INCIDENTE] {r.hostname} RECUPERADO')
                sla_counter[r.id] = 0; sla_active[r.id] = False
            ...
            if anomaly and not (res is None or res is False):
                ...
                if sla_counter[r.id] >= sla_pings and not sla_active.get(r.id):
                    ...
                    Incident.objects.create(router=r, type='sla', started_at=djtime.now(), detail=f'{sla_pings} pings >{threshold}ms')
                    print(f'[SLA] {r.hostname} ALERTA')
            else:
                if anomaly is False:
                    sla_counter[r.id] = 0
                    if sla_active.get(r.id): sla_active[r.id] = False
